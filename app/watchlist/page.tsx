            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  className="bg-card border border-border rounded-lg pl-9 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-blue-500 w-full max-w-xs"
                />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-blue-500 mt-1">
                {watchlistTokens.filter((t) => t.alerts).length}
              </p>
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-green-500 mt-1">
                {watchlistTokens.filter((t) => t.isPositive).length}
              </p>
            ...
            <div className="bg-card border border-border rounded-lg p-4">
              ...
              <p className="text-2xl font-bold text-red-500 mt-1">
                {watchlistTokens.filter((t) => t.riskLevel === 'HIGH RISK').length}
              </p>
            ...
            <div className="lg:col-span-2 bg-card border border-border rounded-lg">
              <div className="p-4 border-b border-border flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button className="px-3 py-1.5 bg-background text-muted-foreground text-xs rounded-lg hover:text-foreground transition-colors">
                    Starred
                  </button>
                  <button className="px-3 py-1.5 bg-background text-muted-foreground text-xs rounded-lg hover:text-foreground transition-colors">
                    Low Risk
                  ...
                  <tbody className="divide-y divide-border">
                    {watchlistTokens.map((token) => (
                      <tr key={token.id} className="hover:bg-muted/50 transition-colors">
                        <td className="p-4">
                          <button
                            className={`${token.starred ? 'text-yellow-500' : 'text-muted-foreground'} hover:text-yellow-500 transition-colors`}
                          >
                        ...
                        <td className="p-4">
                          <span
                            className={`text-xs px-2 py-1 rounded ${getRiskColor(token.riskLevel)}`}
                          >
                            {token.riskLevel}
                          </span>
                        </td>
                        <td className="p-4">
                          <div className="flex items-center gap-1">
                            <button
                              className={`p-1.5 rounded hover:bg-muted transition-colors ${token.alerts ? 'text-blue-500' : 'text-muted-foreground'}`}
                            >
                              {token.alerts ? (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-red-500">
                            Trust Score: {token.trustScore}/100
                          </p>
                        </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-foreground font-semibold">
                      {watchlistTokens.filter((t) => t.verified).length}/{watchlistTokens.length}
                    </span>
                  </div>
                </div>
              </div>
